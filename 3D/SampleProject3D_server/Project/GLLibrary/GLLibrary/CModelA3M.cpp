...
CA3MNode::CA3MNode():
	m_no(0), m_bone_no(-1),m_name(""), m_node_type(A3M::eNode), m_visibility(true), m_bind(false), mp_mesh(nullptr),
	mp_child(nullptr), mp_parent(nullptr), mp_next(nullptr), mp_prev(nullptr)
{
}

CA3MNode::CA3MNode(int no,const char* name):
	m_no(no), m_bone_no(-1),m_node_type(A3M::eNode), m_visibility(true), m_bind(false), mp_mesh(nullptr),
	mp_child(nullptr), mp_parent(nullptr), mp_next(nullptr), mp_prev(nullptr) 
{
...
CA3MNode::CA3MNode(const CA3MNode& n) :
	...
	m_pos(n.m_pos), m_rot(n.m_rot), m_scale(n.m_scale), m_bind(n.m_bind), m_bind_matrix(n.m_bind_matrix), m_visibility(n.m_visibility), mp_mesh(n.mp_mesh),
	mp_child(nullptr), mp_parent(nullptr), mp_next(nullptr), mp_prev(nullptr) {
...
void CA3MMesh::Draw(std::vector<CMaterial*>& materialList, const CShader* shader)
{
...
void CModelA3M::DrawMesh(CA3MNode* node,const CMatrix&view_matrix)
{
	...
	if (node->mp_mesh) {
		...
		//}
		m->Draw(m_material_list, m_shader);
	}
...
CModelA3M::CModelA3M(): mp_root(nullptr),m_animation(nullptr), m_bone_num(0), m_bone_matrix(nullptr), m_animation_size(0)
{
}
...
void CModelA3M::operator=(const CModelA3M& m)
{
	...
	if(m_animation_size>0) m_animation = new CA3MAnimation(&m_animation_list);
	m_col_cut = m.m_col_cut;
}
CModelA3M::CModelA3M(const CModelObj& m) : m_animation(nullptr), m_bone_num(0), m_bone_matrix(nullptr), m_animation_size(0),m_enable_animation(false)
{
	...
	idx += sizeof(int);
	unsigned int	vertex_type = A3M::eStaticMesh;
	memcpy(&buf[idx], &vertex_type, sizeof(vertex_type));
	idx += sizeof(int);
...
bool CModelA3M::DuplicateFBXAnimation(FbxNode* pNode,const CModelFBX::AnimationInfo *info,int anim_num)
{
	...
	FbxAMatrix matA;
		...
		aa->m_size = info->m_end - info->m_start;
		aa->m_keys = new CA3MAnimationKey[aa->m_size];
		...
		for (; key_frame < info->m_end; key_frame++, k++) {
			frame++;
			time.SetTime(0, 0, 0, (int)key_frame, 0, 0, FbxTime::eFrames60);
		...
		if (m_animation_list.size() <= anim_num) {
			...
			set->m_start = 0;
			set->m_fps = FbxTime::GetFrameRate(FbxTime::eFrames60);
...
CModelA3M::CModelA3M(const CModelFBX& m):m_animation(nullptr), m_bone_num(0), m_bone_matrix(nullptr), m_animation_size(0)
{
	...
	if (m.GetAnimationSize() > 0) {
		...
		m_animation = new CA3MAnimation(&m_animation_list);
...
bool CModelA3M::AddAnimationFBX(const char* filepath)
{
	...
	for (int i = 0; i < anim_size; i++) {
		if (DuplicateFBXAnimation(lScene->GetRootNode(),
			&CModelFBX::AnimationInfo(lSdkManager, lScene,
				lScene->GetSrcObject<FbxAnimStack>(i)), m_animation_size))
			m_animation_size++;
	}
	...
	if (!m_animation && m_animation_size > 0) {
		m_animation = new CA3MAnimation(&m_animation_list);
		m_enable_animation = true;
	}
...
CA3MNode* CModelA3M::DuplicateXMesh(CFrameBone* pNode, int& no, CA3MNode* parent, CA3MNode* prev)
{
	CA3MNode* node = new CA3MNode();
	...
	if (pNode->GetChild()) {
		DuplicateXMesh((CFrameBone*)pNode->GetChild(), no, node, nullptr);
	}
	CA3MNode* child = node;
	if (pNode->GetNext()) {
		child = DuplicateXMesh((CFrameBone*)pNode->GetNext(), no, node->mp_parent, child);
	}
...
CModelA3M::CModelA3M(const CModelX& m)
{
	...
	m_col_cut->m_max = CVector3D(FLT_MIN, FLT_MIN, FLT_MIN);
	m_col_cut->m_min = CVector3D(FLT_MAX, FLT_MAX, FLT_MAX);
	mp_root = DuplicateXMesh(m.pFrameRoot, no, nullptr, nullptr);
	CreateCollisionIndex(4, 4, 4);
	m_bone_num = no;
	m_bone_matrix = new CMatrix[m_bone_num];
	...
	*/
	if (pugi::xml_node boneinfo_node = pNode.child("BoneInfo")) {
		...
		for (auto n = bones.begin(); n != bones.end(); n++, b++) {
			...
			ReadMatrix(&bone_node->m_offset, *b);
...
bool CModelA3M::Load(const char* filePath, int cut_x, int cut_y, int cut_z)
{
	...
	if(m_bone_num > 0) m_bone_matrix = new CMatrix[m_bone_num];
...
void CModelA3M::Render(CMatrix& m)
{
	...
	if (m_bone_matrix) {
		int MatrixLocation = glGetUniformLocation(m_shader->GetProgram(), "Transforms");
		glUniformMatrix4fv(MatrixLocation, m_bone_num, GL_FALSE, m_bone_matrix[0].f);
	}
...
bool CModelA3M::isAnimationEnd()
{
	...
	return m_animation[0].isAnimationEnd();
}

int CModelA3M::GetAnimationJam()
{
	if (!m_animation) return 0;
	return m_animation[0].GetAnimationJam();
}

void CModelA3M::SetAnimationSpeed(float s)
{
	if (!m_animation) return;
	m_animation[0].SetAnimationSpeed(s);
}

void CModelA3M::ChangeAnimation(int jam, bool loop, bool check, float time, int blendFrame)
{
	if (!m_animation || jam >= m_animation_size) return;
	m_animation[0].ChangeAnimation(jam,loop,check,time,blendFrame);
}
...
void CA3MAnimation::ChangeAnimation(int jam, bool loop, bool check, float time, int blend_frame)
{
	if (check && jam == m_jam) return;
	m_jam = jam;
