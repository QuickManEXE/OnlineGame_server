...
void MainLoop(void) {
	...
	if (HOLD(CInput::eUp)) {
		...
		pos += dir * 1.0f;
		//0番のアニメーションを再生
		model_a3m.ChangeAnimation(1);
	} else {
		//1番のアニメーションを再生
		model_a3m.ChangeAnimation(0);
	}
	//アニメーションを更新
	model_a3m.UpdateAnimation();


	//座標の設定	x  y  z
	//				CVector3D
	model_a3m.SetPos(pos);
	//回転の設定	x  y  z
	//				CVector3D
	model_a3m.SetRot(rot);

	//大きさの設定	x  y  z
	//				CVector3D
	model_a3m.SetScale(0.5f,0.5f,0.5f);
	model_a3m.Render();
...
//	CLight::SetRadiationAngle(1, Utility::DgreeToRadian(5.0f));
	...
	CCamera::GetCamera()->LookAt(	CVector3D(100,100,100),
									CVector3D(0,10,0),
	...
	//Wepon/Sword/Sword.obj
	//を読み込む
	//ADD_RESOURCE("Model", CModel::CreateModel("Model/model.obj"));
	...
	//読み込んだモデルを複製する
	//model = COPY_RESOURCE("Model", CModelObj);
	model_obj = COPY_RESOURCE("Sword", CModelObj);
	model_a3m = COPY_RESOURCE("Antman", CModelA3M);
...
void Release()
{
	...
	CResourceManager::GetInstance()->Add("Model", CModel::CreateModel("Model./sword"));
}
