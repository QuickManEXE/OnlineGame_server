...
void MainLoop(void) {
	...
	//カメラの位置を設定
	CVector3D eye = camera_at + CVector3D(0,10,-20);
	//カメラの位置と注視点からカメラを設定する
	//							視点、注視点、カメラの上方向ベクトル（今は気にしない(0,1,0)に設定）
	CCamera::GetCurrent()->LookAt(eye, camera_at, CVector3D(0, 1, 0));
