...
void Init(void)
{
	...
	//ランダムの位置に生成
	for (int i = 0; i < 5; i++) {
		float distance = Utility::Rand(10, 40);
		float ang = Utility::Rand(-M_PI, M_PI);
		Base::Add(new TemplateObject(CVector3D(sin(ang), 0, cos(ang)) * distance, -ang));
	}
